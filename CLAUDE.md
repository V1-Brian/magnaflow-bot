...
### Engine ambiguity mislabeled as trim — root cause and fix (2026-07-23)
Live bug report: asking about a "2021 Jeep Wrangler" with no trim, the bot asked "Rubicon or EcoDiesel?" as if choosing a trim — but EcoDiesel is an engine, not a trim, and a real Rubicon can have either engine.
- **Root cause:** the catalog literally stored `"EcoDiesel"` / `"High Altitude EcoDiesel"` in the `submodel` field for the 2021/2022 diesel Wrangler entries. This was a data workaround, not a coincidence — `fitment.js`'s ambiguity resolution only formally tracked four dimensions (trim, body style, drive type, engine config), with no dimension for engine **displacement** on its own. The Wrangler's diesel and gas variants share the same `engine_config` ("V6"), so displacement was the only field that actually distinguished them, and nothing was watching it — hence the submodel workaround. (Checked for scope: the Ram 1500 Tradesman and F-250 Lariat multi-engine cases happened to work already, purely because their engine_config values *also* differ by coincidence — this was a real gap, not a one-off.)
- **Fix:** `fitment.js` now tracks a single merged `engine` ambiguity dimension (displacement + config combined, e.g. `"3.0L V6 EcoDiesel"`) instead of engine_config alone — merged rather than added alongside, so a vehicle differing in both displacement and config (Ram Tradesman, F-250 Lariat) gets asked once, not twice. Catalog data corrected: the two Wrangler diesel rows now have `submodel: null` and `engine_config: "V6 EcoDiesel"` (matching the naming convention already used for Ford's Power Stroke diesel entries).
- **Second bug found while verifying the fix end-to-end:** the engine-config SQL filter was an exact-string match, so when a customer answered the new "3.6L V6 or 3.0L V6 EcoDiesel?" question with something like "the ecodiesel one," extraction correctly picked up `engineLiters: 3.0` but only inferred `engineConfig: "V6"` (not the full "V6 EcoDiesel") — an exact match against "V6 EcoDiesel" then failed and wrongly returned "no confirmed match" for a vehicle that does fit. Changed the engine_config filter from exact-equality to a contains-match — verified safe against every distinct engine_config value in the catalog (every base value like "V6"/"V8"/"I4" is a genuine prefix of its longer variant, no unrelated cross-matches). Confirmed fixed via the realistic 2-turn conversation (bot asks the engine question, customer replies naturally, correct SKU 19505 returned).
- **Changing `submodel` to null changed the vehicle's natural key** (`seed.js`'s upsert key is year+make+model+submodel+engine_liters), so re-seeding created new vehicle/fitment rows instead of updating the old ones — left two orphaned rows (old vehicle + fitment entries still carrying the stale `"EcoDiesel"` submodel) live in the Render Postgres DB with no application-level cleanup. Manually deleted the orphaned `fitment_qualifiers`/`fitment`/`vehicles` rows directly against the live DB. **If a future catalog edit changes `submodel` (or any other field in `vehicleKey()`) on an existing row, check for this same orphaning** — `seed.js` has no automatic cleanup for vehicles whose natural key changed out from under it.
- Conversational test suite re-run after all of the above: 11/11 passing, no regressions.

### Client screenshot review — 3 issues, all fixed (2026-07-23)
Client sent screenshots of a live test: (1) the opening message only asked about the vehicle, not part-type preference; (2) the engine-choice follow-up showed as plain bullet text, not clickable; (3) the client realized they'd missed the cat-back/axle-back distinction from the earlier fix because the full write-up plus 3 duplicate part cards made the message too long to read closely.
- **Opening message was hardcoded in the frontend, bypassing the backend entirely.** The part-type-invitation prompt change earlier today only affects Claude-generated replies — but `frontend/src/components/ChatWidget.jsx`'s initial greeting is a static string shown before any `/chat` call, so it never picked up that change. Fixed the hardcoded string directly.
- **Made vehicle-identifying follow-ups (trim, body style, drive type, engine) clickable, not just genuine fitment qualifiers.** Previously only qualifiers like leaf/coil-spring suspension got buttons (`claude.js`'s `clarifyingOptions` check was `kind === 'qualifier'`) — vehicle fields were considered things the customer "already knows" and left as typed follow-ups. Changed to render all pending items as buttons: both are always a small closed list pulled from the catalog, and a clicked label round-trips verbatim into the next extraction pass — which also hardens against the exact class of bug found earlier today (a typed reply not mapping cleanly onto the stored value).
- **Stopped duplicating full part details in prose.** `PartCard.jsx` already renders SKU, series, full description, price, sound level, install difficulty, and the product link for every part — but `system.js` also told Claude to include all of that in its written reply, so every result appeared in full twice in a row (once in text, once as a card). This is what buried the cat-back/axle-back distinction added earlier today. `system.js` now tells Claude to skip restating spec details already on the card and keep text to what the card can't show: a brief intro, the plain-English sound/install translation, and the inter-part distinction. First attempt caused Claude to literally write "(card below)" as a placeholder — tightened the instruction to forbid referencing the card's existence at all, just write naturally.
- Verified end-to-end: engine question now returns clickable buttons; clicking through to "EcoDiesel" correctly resolves SKU 19505; the multi-part Wrangler Rubicon response dropped from ~1700 to ~1400 characters with the distinction still clearly explained and no more placeholder text. Conversational test suite re-run: 11/11 passing.
- **Added 4 regression cases to `backend/test/conversations/cases.js`** locking in today's two logic fixes: the Wrangler engine-disambiguation case (not-yet-answered, EcoDiesel-answered, gas-answered) and the multi-part-type Rubicon case (all 3 SKUs returned together, not narrowed to one). Deliberately chosen because both protect fitment-*matching logic* (engine ambiguity resolution, multi-part-type presentation) that a real ACES/PIES import will exercise even more than this 88-vehicle demo catalog does — not one-off demo trivia. Same caveat as every other case already in the suite: they hardcode today's real SKUs, so a future catalog swap may need the expected values updated, but the behavior being protected persists. 15/15 passing.

### Card redesign + animated typing indicator (2026-07-23)
Client feedback after the above: even with duplication removed, the per-part text was still too long, and the static "..." loading state should be more alive.
- **`frontend/src/components/PartCard.jsx` redesigned** — added a part-type badge (Cat-Back/Axle-Back/etc., previously only distinguishable by reading the description text) and replaced the full paragraph `description` with a one-line summary: structured `part_attributes` (pipe diameter, exit config) when present, else the first clause of the description. Price/sound/install stay inline; the link is now "Full details on MagnaFlow" to reinforce that the full writeup lives there, not in the chat.
- **`system.js` tightened again** — first attempt at a multi-part reply asked Claude to write one combined comparison paragraph across all parts, which caused a real hallucination: it described "two Overland axle-backs" when there's only one (the other Overland is a cat-back). Fixed by requiring one short clause per part, each explicitly tied to its own SKU, plus an explicit warning that miscounting part types is a factual error, not a style issue. Also added a carve-out: the "don't restate full specs" rule only applies to the *initial* presentation — if the customer later asks for more detail on a specific part, Claude should answer fully from the fitment data already in conversation history.
- **Loading indicator replaced** — `TypingIndicator.jsx` (new) renders a 3-dot CSS bounce animation (`@keyframes` in `index.css`, no new dependency) in place of the old static `MessageBubble` showing literal "...".
- **Verified visually, not just via the conversational suite** — installed frontend deps (weren't installed yet) and Playwright/Chromium in `qa/` (also not yet installed), ran both dev servers locally, and drove the app with a headless-Chromium script to screenshot the opening message, the mid-response typing indicator, and the final card layout. Confirmed the redesigned cards render cleanly (badge, short summary, price/sound/install, link) and all 3 fit without excessive scrolling.
- **Known flake observed, not a regression:** during testing, one run out of ~13 had the response-generation pass ask about engine/lift status despite `fitmentResults` already being populated with 3 correct matches — a direct violation of the "present results, full stop" rule. The data layer (`lookupParts`) was confirmed correct and deterministic every time; this is the model occasionally not following an explicit instruction, consistent with the non-determinism already logged under "Simulated-customer test harness" above. Not something today's changes introduced (reproduced independently of the prose-length changes), but worth another look if a client demo hits it live.
- Conversational suite re-run after all of the above: 15/15 passing.
